...
fn open_pending_child_webviews_skips_unmapped_child_webview_ids() {
    let mut app = GraphBrowserApp::new_for_testing();
    let mapped_node = app.add_node_and_sync(
        "https://example.com/mapped".to_string(),
        euclid::default::Point2D::new(0.0, 0.0),
    );
...
fn deferred_child_webview_retries_and_opens_via_frame_routed_intent() {
    ...
    let deferred =
        super::open_pending_child_webview_nodes(&mut app, &mut frame_intents, vec![child_webview]);
    ...
    let deferred_after_mapping =
        super::open_pending_child_webview_nodes(&mut app, &mut frame_intents, deferred);
    ...
    assert!(
        node_pane_exists,
        "child node pane should open after routed retry"
    );
}
