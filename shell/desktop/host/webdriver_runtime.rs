...
mod noop {
    ...
    use servo::{
        ...
        Preferences, WebDriverLoadStatus, WebViewId,
    };
    use url::Url;
    ...
    impl WebDriverRuntime {
        ...
        pub(crate) fn complete_traversal(&self, _traversal_id: servo::TraversalId) {}

        pub(crate) fn finish_input_event(&self, _id: InputEventId) {}
        ...
        pub(crate) fn block_load_status_if_any(&self, _webview_id: WebViewId) {}

        pub(crate) fn show_embedder_control(
            &self,
            _webview_id: WebViewId,
            _embedder_control: EmbedderControl,
        ) {
        }
        ...
        pub(crate) fn hide_embedder_control(
            &self,
            _webview_id: WebViewId,
            _embedder_control_id: EmbedderControlId,
        ) {
        }
    }
}
...
#[cfg(not(feature = "webdriver"))]
pub(crate) use noop::WebDriverRuntime;
...
mod real {
    ...
    use crate::shell::desktop::runtime::registries::{
        ...
        CHANNEL_HOST_WEBDRIVER_LOAD_STATUS_BLOCKED,
    ...
    impl WebDriverEmbedderControls {
        ...
        pub(crate) fn respond_to_active_simple_dialog(
            ...
            let Some(&EmbedderControl::SimpleDialog(simple_dialog)) = controls.last().as_ref()
            else {
                return Err(());
            };
            ...
            match action {
                ...
                WebDriverUserPromptAction::Ignore => {
                    unreachable!("Should have returned early above")
                }
            }
            Ok(result_text)
        }
        ...
        pub(crate) fn message_of_newest_dialog(&self, webview_id: WebViewId) -> Option<String> {
            ...
            match embedder_controls.get(&webview_id)?.last()? {
                EmbedderControl::SimpleDialog(simple_dialog) => {
                    Some(simple_dialog.message().into())
                }
                _ => None,
            }
        }

        pub(crate) fn set_prompt_value_of_newest_dialog(
            &self,
            webview_id: WebViewId,
            text: String,
        ) {
    ...
    impl WebDriverRuntime {
            ...
            while let Ok(msg) = receiver.try_recv() {
                match msg {
                    ...
                    WebDriverCommandMsg::GetAlertText(webview_id, response_sender) => {
                        let response =
                            match self.embedder_controls.message_of_newest_dialog(webview_id) {
                                Some(text) => Ok(text),
                                None => Err(()),
                            };
        ...
        fn try_handle_ux_bridge_script_command(
            ...
            let result = handle_ux_bridge_script_payload(payload, |intent| {
                ...
                window.notify_workbench_intent_request(intent);
                Ok(())
            });
    ...
    mod tests {
        ...
        fn ux_bridge_query_script_reports_missing_snapshot() {
            ...
            let response =
                handle_ux_bridge_script_payload(r#"{"command":"GetUxSnapshot"}"#, |_| {
