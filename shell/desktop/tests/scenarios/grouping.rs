...
fn tab_container_count_for_nodes(harness: &TestRegistry, nodes: &[crate::graph::NodeKey]) -> usize {
...
fn dismiss_tile_demotes_lifecycle_and_preserves_edges() {
    ...
    harness
        .app
        .apply_reducer_intents([GraphIntent::CreateUserGroupedEdge {
            from: a,
            to: b,
            label: None,
        }]);
...
fn cold_node_reactivated_joins_existing_tab_group() {
    ...
    assert!(
        !has_tile(&harness, b),
        "b should have no tile after dismiss"
    );
    ...
    assert!(
        has_tile(&harness, b),
        "b should have a tile after reactivation"
    );
...
fn reconcile_graphlet_merges_tiles_from_different_tab_containers() {
    ...
    let a_pane = harness
        .tiles_tree
        .tiles
        .insert_pane(TileKind::Node(a.into()));
    let a_tabs = harness.tiles_tree.tiles.insert_tab_tile(vec![a_pane]);
    let b_pane = harness
        .tiles_tree
        .tiles
        .insert_pane(TileKind::Node(b.into()));
    ...
    assert!(
        pending
            .iter()
            .any(|i| matches!(i, WorkbenchIntent::ReconcileGraphletTiles { .. })),
...
fn remove_from_graphlet_action_retracts_durable_edges_only() {
    ...
    assert!(
        !has_grouped,
        "UserGrouped edge should be retracted by RemoveFromGraphlet"
    );
...
fn warm_select_action_dispatches_open_intent_for_cold_selected_nodes() {
    ...
    assert_eq!(
        open_nodes.len(),
        2,
        "warm-select should only target cold nodes"
    );
}
