...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
enum SemanticTabTransitionTarget {
    Restored,
    Collapsed,
}
...
fn confirm_semantic_tab_transition(
    ...
    group_id: Uuid,
    focus_pane: PaneId,
    expected: SemanticTabTransitionTarget,
) -> bool {
    ...
    let Some(group) = semantics
        .tab_groups
        .iter()
        .find(|group| group.group_id == group_id && group.pane_ids.contains(&focus_pane))
    else {
        return false;
    };

    let member_tile_ids: Vec<_> = group
        .pane_ids
        .iter()
        .filter_map(|pane_id| tile_id_for_pane(tiles_tree, *pane_id))
        .collect();
    if member_tile_ids.len() != group.pane_ids.len() {
        return false;
    }

    let attached_member_count = member_tile_ids
        .iter()
        .filter(|tile_id| tile_is_attached(tiles_tree, **tile_id))
        .count();
    let focus_tile_id = match tile_id_for_pane(tiles_tree, focus_pane) {
        Some(tile_id) => tile_id,
        None => return false,
    };
    let focus_is_active = tiles_tree.active_tiles().contains(&focus_tile_id);
    let exact_tabs_container =
        tabs_container_for_exact_runtime_members(tiles_tree, &member_tile_ids);
    ...
    match (
        ...
        (
            ...
            affordance_group_id == group_id
                && attached_member_count == 1
                && exact_tabs_container.is_none()
                && focus_is_active
        }
        (
            ...
            affordance_group_id == group_id
                && attached_member_count == group.pane_ids.len()
                && exact_tabs_container.is_some()
                && focus_is_active
        }
        _ => false,
    }
...
pub(crate) fn collapse_semantic_tab_group_to_pane_rest(
    tiles_tree: &mut Tree<TileKind>,
    ...
    if ordered_tile_ids.len() <= 1 {
        let focus_pane_id = group
            .active_pane_id
            .or_else(|| group.pane_ids.first().copied());
        graph_app.set_current_frame_tab_semantics(Some(semantics));
        return focus_pane_id.is_some_and(|pane_id| {
            focus_pane(tiles_tree, pane_id)
                && confirm_semantic_tab_transition(
                    tiles_tree,
                    graph_app,
                    group_id,
                    pane_id,
    ...
    let Some(tabs_id) = tabs_container_for_exact_runtime_members(tiles_tree, &ordered_tile_ids)
    else {
        graph_app.set_current_frame_tab_semantics(Some(semantics));
        return ordered_tile_ids
            .iter()
            .copied()
            .find(|tile_id| tile_is_attached(tiles_tree, *tile_id))
            .is_some();
    };

    let active_pane = group
        .active_pane_id
        .or_else(|| match tiles_tree.tiles.get(tabs_id) {
            Some(Tile::Container(Container::Tabs(tabs))) => {
                tabs.active
                    .and_then(|active_tile| match tiles_tree.tiles.get(active_tile) {
    ...
    let active_pane = active_pane
        .filter(|pane_id| group.pane_ids.contains(pane_id))
        .or_else(|| group.pane_ids.first().copied());
    let Some(active_pane) = active_pane else {
        return false;
    };
    group.active_pane_id = Some(active_pane);

    let active_tile_id = match tile_id_for_pane(tiles_tree, active_pane) {
        Some(tile_id) => tile_id,
        None => return false,
    };
    let sibling_tile_ids: Vec<_> = ordered_tile_ids
        .into_iter()
        .filter(|tile_id| *tile_id != active_tile_id && tile_is_attached(tiles_tree, *tile_id))
        .collect();
    for tile_id in sibling_tile_ids {
        let _ = detach_tile_for_reparent(tiles_tree, tile_id);
    }

    let _ = focus_pane(tiles_tree, active_pane);
    tile_runtime::refresh_node_pane_render_modes(tiles_tree, graph_app);
    graph_app.set_current_frame_tab_semantics(Some(semantics));
    confirm_semantic_tab_transition(
        tiles_tree,
...
pub(crate) fn restore_pane_to_semantic_tab_group(
    tiles_tree: &mut Tree<TileKind>,
    graph_app: &mut GraphBrowserApp,
    pane: PaneId,
    group_id: Uuid,
    ...
    let Some(group) = semantics
        .tab_groups
        .iter()
        .find(|group| group.group_id == group_id && group.pane_ids.contains(&pane))
    else {
        return false;
    };
    ...
    if ordered_tile_ids.len() <= 1 {
        graph_app.set_current_frame_tab_semantics(Some(semantics));
        return focus_pane(tiles_tree, pane)
            && confirm_semantic_tab_transition(
                tiles_tree,
                graph_app,
                group_id,
    ...
    if let Some(tabs_id) = tabs_container_for_exact_runtime_members(tiles_tree, &ordered_tile_ids) {
        let active_tile_id = group
            .active_pane_id
            .filter(|pane_id| group.pane_ids.contains(pane_id))
            .unwrap_or(pane);
        let active_tile_id = tile_id_for_pane(tiles_tree, active_tile_id);
        if let Some(Tile::Container(Container::Tabs(tabs))) = tiles_tree.tiles.get_mut(tabs_id) {
            if let Some(active_tile_id) = active_tile_id {
                tabs.set_active(active_tile_id);
            }
        }
        graph_app.set_current_frame_tab_semantics(Some(semantics));
        tile_runtime::refresh_node_pane_render_modes(tiles_tree, graph_app);
    ...
    let mut attached_tile_ids: Vec<_> = ordered_tile_ids
        .iter()
        .copied()
        .filter(|tile_id| tile_is_attached(tiles_tree, *tile_id))
        .collect();
    attached_tile_ids.sort_by_key(|tile_id| tile_id.0);
    let anchor_tile = attached_tile_ids
        .first()
        .copied()
        .or_else(|| ordered_tile_ids.first().copied());
    let Some(anchor_tile) = anchor_tile else {
        return false;
    };
    let anchor_parent = tiles_tree.tiles.parent_of(anchor_tile);
    let anchor_was_root = tiles_tree.root() == Some(anchor_tile);

    for tile_id in attached_tile_ids
        .iter()
        .copied()
        .filter(|tile_id| *tile_id != anchor_tile)
    {
        let _ = detach_tile_for_reparent(tiles_tree, tile_id);
    }

    let tabs_id = tiles_tree.tiles.insert_tab_tile(ordered_tile_ids.clone());
    let active_tile_id = group
        .active_pane_id
        .filter(|pane_id| group.pane_ids.contains(pane_id))
        .unwrap_or(pane);
    ...
    if let Some(parent_id) = anchor_parent {
        replace_child_in_parent(tiles_tree, parent_id, anchor_tile, tabs_id);
    } else if anchor_was_root || tiles_tree.root().is_none() {
        tiles_tree.root = Some(tabs_id);
    } else {
        return false;
    }
...
pub(crate) fn open_or_focus_frame_tile_group(
    ...
    let Some((mut tab_tiles, _, _hint_tabs)) = persistence_ops::materialize_frame_tile_group_tabs(
        graph_app,
        frame_anchor,
        &mut tiles_tree.tiles,
    ) else {
        log::debug!(
            "tile_view_ops: open_or_focus_frame_tile_group: frame anchor {:?} could not materialize tabs",
            frame_anchor
        );
        return;
    };
...
fn find_frame_tile_group(
    tiles_tree: &Tree<TileKind>,
    graph_app: &GraphBrowserApp,
    frame_anchor: NodeKey,
    member_keys: &[NodeKey],
) -> Option<TileId> {
    if let Some(group_id) = graph_app
        .workspace
        .graph_runtime
        .frame_tile_groups
        .iter()
    ...
    for &member_key in member_keys {
        ...
        let mut ancestor = tiles_tree.tiles.parent_of(member_tile_id);
        while let Some(parent_id) = ancestor {
            ...
            ancestor = tiles_tree.tiles.parent_of(parent_id);
        }
    }
    None
}
...
fn focus_member_tile_in_group(
    ...
    let focus_child_tile = direct_child_tab_containing_node(tiles_tree, group_id, focus_key);
    let Some(focus_child_tile) = focus_child_tile else {
        return;
    };
    if let Some(Tile::Container(Container::Tabs(tabs))) = tiles_tree.tiles.get_mut(group_id) {
        tabs.set_active(focus_child_tile);
    }
}

fn direct_child_tab_containing_node(
    tiles_tree: &Tree<TileKind>,
    group_id: TileId,
    focus_key: NodeKey,
) -> Option<TileId> {
    let Tile::Container(Container::Tabs(tabs)) = tiles_tree.tiles.get(group_id)? else {
        return None;
    };
    ...
    tabs.children
        .iter()
        .copied()
        .find(|child| tile_subtree_contains_node(tiles_tree, *child, focus_key))
}

fn tile_subtree_contains_node(
    tiles_tree: &Tree<TileKind>,
    tile_id: TileId,
    focus_key: NodeKey,
) -> bool {
    match tiles_tree.tiles.get(tile_id) {
        Some(Tile::Pane(kind)) => tile_matches_node(kind, focus_key) && !kind.is_floating(),
        Some(Tile::Container(Container::Tabs(tabs))) => tabs
            .children
            .iter()
            .copied()
            .any(|child| tile_subtree_contains_node(tiles_tree, child, focus_key)),
...
mod tests {
    ...
    #[test]
    fn semantic_tab_group_can_collapse_and_restore_live_tree() {
        let mut app = GraphBrowserApp::new_for_testing();
        let a = app.add_node_and_sync(
            "https://semantic-live-a.example".into(),
            euclid::default::Point2D::new(0.0, 0.0),
        );
        let b = app.add_node_and_sync(
            "https://semantic-live-b.example".into(),
        ...
        let mut tree = Tree::new("semantic_tab_restore_roundtrip", root, tiles);

        let semantics = persistence_ops::derive_runtime_frame_tab_semantics_from_tree(&tree)
            .expect("semantics");
        ...
        let pane_b = match tree.tiles.get(b_tile) {
            Some(Tile::Pane(tile)) => tile.pane_id(),
            other => panic!("expected pane tile for b, got {other:?}"),
        };
        app.set_current_frame_tab_semantics(Some(semantics));

        assert!(collapse_semantic_tab_group_to_pane_rest(
            &mut tree,
            &mut app,
            group.group_id
        ));
        assert!(
            !matches!(
                tree.tiles.get(tree.root().expect("root after collapse")),
                Some(Tile::Container(Container::Tabs(_)))
            ),
            "collapse should leave pane-rest form at the root"
        );
        let attached_count = [pane_a, pane_b]
            .into_iter()
            .filter(|pane_id| {
                tile_id_for_pane(&tree, *pane_id)
                    .is_some_and(|tile_id| tile_is_attached(&tree, tile_id))
            })
            .count();
        assert_eq!(attached_count, 1);
        let active_after_collapse = tree.active_tiles().into_iter().next().expect("active tile");
        let active_pane_after_collapse = match tree.tiles.get(active_after_collapse) {
            Some(Tile::Pane(tile)) => tile.pane_id(),
            other => panic!("expected active pane after collapse, got {other:?}"),
        };
        assert_eq!(active_pane_after_collapse, pane_a);

        assert!(restore_pane_to_semantic_tab_group(
            &mut tree,
            &mut app,
            pane_b,
            group.group_id
        ));
        match tree.tiles.get(tree.root().expect("root after restore")) {
            Some(Tile::Container(Container::Tabs(tabs))) => {
                assert_eq!(tabs.children.len(), 2);
            }
            other => panic!("expected restored tabs root, got {other:?}"),
        }
        let active_after_restore = tree.active_tiles();
        assert!(active_after_restore.contains(&b_tile));
        assert_eq!(
            app.current_frame_tab_semantics()
    ...
    #[test]
    fn restore_semantic_tab_group_requires_confirmed_runtime_shape() {
        let mut app = GraphBrowserApp::new_for_testing();
        let a = app.add_node_and_sync(
            "https://semantic-confirm-a.example".into(),
            euclid::default::Point2D::new(0.0, 0.0),
        );
        let b = app.add_node_and_sync(
            "https://semantic-confirm-b.example".into(),
            euclid::default::Point2D::new(80.0, 0.0),
        );
        ...
        let b_tile = tiles.insert_pane(TileKind::Node(b.into()));
        let root = tiles.insert_tab_tile(vec![a_tile, b_tile]);
        let mut tree = Tree::new("semantic_tab_restore_confirmation", root, tiles);
    ...
    #[test]
    fn semantic_tab_affordance_and_toggle_intent_follow_runtime_state() {
        let mut app = GraphBrowserApp::new_for_testing();
        let a = app.add_node_and_sync(
            "https://semantic-toggle-a.example".into(),
            euclid::default::Point2D::new(0.0, 0.0),
        );
        let b = app.add_node_and_sync(
            "https://semantic-toggle-b.example".into(),
            euclid::default::Point2D::new(80.0, 0.0),
        );
        ...
        let b_tile = tiles.insert_pane(TileKind::Node(b.into()));
        let root = tiles.insert_tab_tile(vec![a_tile, b_tile]);
        let mut tree = Tree::new("semantic_tab_toggle_affordance", root, tiles);
        ...
        assert_eq!(
            semantic_tab_affordance_for_pane(&tree, &app, pane_a),
        ...
        assert!(matches!(
            semantic_tab_toggle_intent_for_pane(&tree, &app, pane_a),
            Some(WorkbenchIntent::CollapseSemanticTabGroupToPaneRest {
                group_id
        ...
        assert!(matches!(
            ...
            Some(WorkbenchIntent::RestorePaneToSemanticTabGroup { pane, group_id })
                if pane == pane_a && group_id == group.group_id
        ));
    }

    #[test]
    fn semantic_tab_queries_include_collapsed_runtime_members() {
        let mut app = GraphBrowserApp::new_for_testing();
        let a = app.add_node_and_sync(
            "https://semantic-collapsed-a.example".into(),
            euclid::default::Point2D::new(0.0, 0.0),
        );
        let b = app.add_node_and_sync(
            "https://semantic-collapsed-b.example".into(),
            euclid::default::Point2D::new(80.0, 0.0),
        );
        ...
        let b_tile = tiles.insert_pane(TileKind::Node(b.into()));
        let root = tiles.insert_tab_tile(vec![a_tile, b_tile]);
        let mut tree = Tree::new("semantic_tab_collapsed_queries", root, tiles);
        ...
        let active_tile = tree.active_tiles().into_iter().next().expect("active tile");
        let key_set = semantic_tab_node_keys(&tree, &app);

        assert_eq!(key_set.len(), 2);
        assert!(key_set.contains(&a));
        assert!(key_set.contains(&b));
        assert_eq!(
            semantic_tab_node_order_for_tile(&tree, &app, active_tile),
            Some(vec![a, b])
        );
    }

    #[test]
    fn semantic_tab_order_in_tiles_preserves_visible_tab_order() {
        let mut app = GraphBrowserApp::new_for_testing();
        let a = app.add_node_and_sync(
            "https://semantic-order-a.example".into(),
            euclid::default::Point2D::new(0.0, 0.0),
        );
        let b = app.add_node_and_sync(
            "https://semantic-order-b.example".into(),
            euclid::default::Point2D::new(80.0, 0.0),
        );
        ...
        let b_tile = tiles.insert_pane(TileKind::Node(b.into()));
        let root = tiles.insert_tab_tile(vec![a_tile, b_tile]);
        let tree = Tree::new("semantic_tab_visible_order", root, tiles);

        let semantics = persistence_ops::derive_runtime_frame_tab_semantics_from_tree(&tree)
            .expect("semantics");
        app.set_current_frame_tab_semantics(Some(semantics));

        let tabs = match tree.tiles.get(root) {
            Some(Tile::Container(Container::Tabs(tabs))) => tabs,
            other => panic!("expected tabs root container, got {other:?}"),
        };
        let mut reordered_children = tabs.children.clone();
        reordered_children.reverse();

        let mut reordered_tiles = tree.tiles.clone();
        if let Some(Tile::Container(Container::Tabs(tabs))) = reordered_tiles.get_mut(root) {
            tabs.children = reordered_children;
            tabs.set_active(b_tile);
        }

        assert_eq!(
            semantic_tab_node_order_for_tile_in_tiles(&reordered_tiles, &app, b_tile),
            Some(vec![b, a])
        );
    }
    ...
    fn open_frame_tile_group_creates_tabs_container_for_all_members() {
        let (mut app, frame_anchor, member_keys) = make_frame_with_members(2);
        let graph_view = GraphViewId::new();
        let mut tiles = Tiles::default();
        let root = tiles.insert_pane(graph_pane(graph_view));
        let mut tree = Tree::new("frame_group_create", root, tiles);

        open_or_focus_frame_tile_group(&mut tree, &mut app, frame_anchor, None);
    ...
    fn open_frame_tile_group_focuses_existing_group_on_second_call() {
        ...
        let group_id_after_first = find_frame_tile_group(&tree, &app, frame_anchor, &member_keys);

        open_or_focus_frame_tile_group(&mut tree, &mut app, frame_anchor, None);
        let node_pane_count_after_second = count_node_panes(&tree);
        let group_id_after_second = find_frame_tile_group(&tree, &app, frame_anchor, &member_keys);
    ...
    fn open_frame_tile_group_with_focus_key_makes_correct_tile_active() {
        ...
        open_or_focus_frame_tile_group(&mut tree, &mut app, frame_anchor, Some(focus_key));
    ...
    #[test]
    fn open_frame_tile_group_materializes_hint_tabs_and_focuses_nested_member() {
        let (mut app, frame_anchor, member_keys) = make_frame_with_members(3);
        app.apply_reducer_intents([GraphIntent::RecordFrameLayoutHint {
            frame: frame_anchor,
            hint: crate::graph::FrameLayoutHint::SplitHalf {
                first: app
                    .domain_graph()
                    .get_node(member_keys[0])
                    .expect("first member")
                    .id
                    .to_string(),
                second: app
                    .domain_graph()
                    .get_node(member_keys[1])
        ...
        let mut tiles = Tiles::default();
        let root = tiles.insert_pane(graph_pane(graph_view));
        let mut tree = Tree::new("frame_group_hint_focus", root, tiles);

        open_or_focus_frame_tile_group(&mut tree, &mut app, frame_anchor, Some(member_keys[1]));

        let group_id = find_frame_tile_group(&tree, &app, frame_anchor, &member_keys)
            .expect("frame tile group should exist");
        let tabs = match tree.tiles.get(group_id) {
            Some(Tile::Container(Container::Tabs(tabs))) => tabs,
        ...
        assert_eq!(hint_info.0, frame_anchor);
