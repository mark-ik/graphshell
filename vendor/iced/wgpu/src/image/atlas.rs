...
impl Atlas {
    ...
    fn upload_allocation(
        ...
        // wgpu 29 dropped `Index<Range>` impls for `BufferViewMut`;
        // ranged writes go through `.slice(range).copy_from_slice(src)`
        // which returns a `WriteOnly<[u8]>` view. Bit-rot patch
        // (2026-05-04) for the iced image feature, which graphshell main
        // doesn't enable so this had drifted from the vendored copy.
