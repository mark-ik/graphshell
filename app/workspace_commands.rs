...
pub(crate) fn set_pending_camera_command(
    ...
    let _ = self.take_pending_app_command(|queued| matches!(queued, AppCommand::CameraCommand { .. }));
...
pub(crate) fn set_pending_keyboard_zoom_request(
    ...
    let _ = self.take_pending_app_command(|command| matches!(command, AppCommand::KeyboardZoom { .. }));
...
pub(crate) fn set_pending_wheel_zoom_delta(
    ...
    let existing = self.take_pending_app_command(|command| matches!(command, AppCommand::WheelZoom { .. }));
...
pub(crate) fn set_pending_workspace_restore_open_request(
    ...
    let _ = self.take_pending_app_command(|command| matches!(command, AppCommand::RestoreWorkspaceOpen { .. }));
...
pub(crate) fn set_pending_unsaved_workspace_prompt(
    ...
    let _ = self.take_pending_app_command(|command| matches!(command, AppCommand::UnsavedWorkspacePrompt { .. }));
...
pub(crate) fn set_pending_choose_workspace_picker(
    ...
    let _ = self.take_pending_app_command(|command| matches!(command, AppCommand::ChooseWorkspacePicker { .. }));
...
pub(crate) fn set_pending_history_workspace_layout_json(&mut self, layout_json: Option<String>) {
    ...
    while let Some(command) = self.workspace.pending_app_commands.pop_front() {
        let retained = match command {
            AppCommand::AddNodeToWorkspace {
                node,
                workspace_name,
            } => self.workspace.domain.graph.get_node(node).map(|_| AppCommand::AddNodeToWorkspace {
                node,
                workspace_name,
            }),
