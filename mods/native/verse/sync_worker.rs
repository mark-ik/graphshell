...
pub struct SyncWorker {
    ...
    discovery_result_tx:
        mpsc::UnboundedSender<Result<Vec<crate::mods::native::verse::DiscoveredPeer>, String>>,
...
impl SyncWorker {
    ...
    pub fn new(
        ...
        intent_tx: mpsc::Sender<QueuedIntent>,
        command_rx: mpsc::Receiver<SyncCommand>,
        discovery_result_tx: mpsc::UnboundedSender<
            Result<Vec<crate::mods::native::verse::DiscoveredPeer>, String>,
        >,
        cancel: CancellationToken,
    ) -> Self {
        Self {
...
impl SyncWorkerHandle {
    ...
    async fn handle_incoming(&self, connecting: iroh::endpoint::Connecting) -> Result<(), String> {
        ...
        let is_trusted = {
            let peers = self
                .trusted_peers
                .read()
                .expect("trusted peer lock poisoned");
        ...
        // Decompress (zstd)
        let decompressed =
            zstd::decode_all(&buf[..]).map_err(|e| format!("zstd decompress failed: {}", e))?;
        ...
        log::info!(
            "Received SyncUnit from {}: {} intents",
            peer_id,
            sync_unit.intents.len()
        );
        ...
        log::info!(
            "Sent SyncUnit to {}: {} intents",
            peer_id,
            our_sync_unit.intents.len()
        );
    ...
    async fn initiate_sync(
        ...
        log::info!(
            "Initiating sync with peer {} for workspace {}",
            peer_id,
            workspace_id
        );
        ...
        if !buf.is_empty() {
            ...
            log::info!(
                "Received response from {}: {} intents",
                peer_id,
                sync_unit.intents.len()
            );
            self.process_incoming_sync_unit(peer_id, sync_unit).await?;
    ...
    async fn build_outgoing_sync_unit(
        ...
        let sync_log = logs
            .get(workspace_id)
            .ok_or_else(|| format!("no sync log for workspace {}", workspace_id))?;

        let intents: Vec<SyncedIntent> = sync_log
            .intents
            .iter()
            .filter(|intent| intent.sequence > peer_vv.get(intent.authored_by))
            .cloned()
            .collect();
    ...
    async fn process_incoming_sync_unit(
        ...
        if access == AccessLevel::ReadOnly && !sync_unit.intents.is_empty() {
            log::warn!(
                "Peer {} is read-only for {} - rejecting mutations",
                peer_id,
                workspace_id
            );
            emit_event(DiagnosticEvent::MessageReceived {
