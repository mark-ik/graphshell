...
fn phase0_registry_normalization_emits_protocol_and_viewer_success_channels() {
    ...
    assert!(
        TestHarness::channel_count(&snapshot, "registry.protocol.resolve_started") > 0,
    ...
    assert!(
        TestHarness::channel_count(&snapshot, "registry.viewer.select_started") > 0,
...
fn phase0_registry_normalization_emits_fallback_channels_for_unknown_scheme_and_extension() {
    ...
    let snapshot = harness.snapshot();
    assert!(
        TestHarness::channel_count(&snapshot, "registry.protocol.resolve_failed") > 0,
        "unknown scheme should emit protocol resolve failed channel"
    );
    assert!(
        TestHarness::channel_count(&snapshot, "registry.protocol.fallback_used") > 0,
        "unknown scheme should emit protocol fallback channel"
    );
    assert!(
        TestHarness::channel_count(&snapshot, "registry.viewer.fallback_used") > 0,
...
fn phase0_registry_cancellation_short_circuits_before_viewer_selection() {
    ...
    assert_eq!(
        TestHarness::channel_count(&snapshot, "registry.viewer.select_started"),
        0,
...
fn phase2_action_registry_omnibox_search_emits_action_channels() {
    ...
    let snapshot = harness.snapshot();
    assert!(
        TestHarness::channel_count(&snapshot, "registry.action.execute_started") > 0,
        "action execute start channel should be emitted"
    );
    assert!(
        TestHarness::channel_count(&snapshot, "registry.action.execute_succeeded") > 0,
        "action execute success channel should be emitted"
    );
    assert_eq!(
        TestHarness::channel_count(&snapshot, "registry.action.execute_failed"),
        0,
...
fn phase2_input_registry_toolbar_submit_binding_emits_resolved_channel() {
    ...
    let snapshot = harness.snapshot();
    assert!(
        TestHarness::channel_count(&snapshot, "registry.input.binding_resolved") > 0,
        "input binding resolved channel should be emitted"
    );
    assert_eq!(
        TestHarness::channel_count(&snapshot, "registry.input.binding_missing"),
        0,
        "input binding missing channel should not be emitted"
    );
    assert_eq!(
        TestHarness::channel_count(&snapshot, "registry.input.binding_conflict"),
        0,
...
fn phase2_lens_registry_default_id_emits_resolve_succeeded_channel() {
    ...
    let snapshot = harness.snapshot();
    assert!(
        TestHarness::channel_count(&snapshot, "registry.lens.resolve_succeeded") > 0,
        "lens resolve succeeded channel should be emitted"
    );
    assert!(
        TestHarness::channel_count(&snapshot, "registry.layout.lookup_succeeded") > 0,
        "layout lookup succeeded channel should be emitted"
    );
    assert!(
        TestHarness::channel_count(&snapshot, "registry.physics.lookup_succeeded") > 0,
        "physics lookup succeeded channel should be emitted"
    );
    assert!(
        TestHarness::channel_count(&snapshot, "registry.theme.lookup_succeeded") > 0,
        "theme lookup succeeded channel should be emitted"
    );
    assert_eq!(
        TestHarness::channel_count(&snapshot, "registry.lens.resolve_failed"),
        0,
...
fn phase2_lens_registry_unknown_id_emits_failed_and_fallback_channels() {
    ...
    let snapshot = harness.snapshot();
    assert!(
        TestHarness::channel_count(&snapshot, "registry.lens.resolve_failed") > 0,
        "lens resolve failed channel should be emitted for unknown id"
    );
    assert!(
        TestHarness::channel_count(&snapshot, "registry.lens.fallback_used") > 0,
...
fn phase2_lens_component_id_resolution_emits_component_fallback_channels() {
    ...
    let snapshot = harness.snapshot();
    assert!(
        TestHarness::channel_count(&snapshot, "registry.physics.lookup_failed") > 0,
        "physics lookup failed channel should be emitted for unknown id"
    );
    assert!(
        TestHarness::channel_count(&snapshot, "registry.physics.fallback_used") > 0,
        "physics fallback channel should be emitted for unknown id"
    );
    assert!(
        TestHarness::channel_count(&snapshot, "registry.layout.lookup_failed") > 0,
        "layout lookup failed channel should be emitted for unknown id"
    );
    assert!(
        TestHarness::channel_count(&snapshot, "registry.layout.fallback_used") > 0,
        "layout fallback channel should be emitted for unknown id"
    );
    assert!(
        TestHarness::channel_count(&snapshot, "registry.theme.lookup_failed") > 0,
        "theme lookup failed channel should be emitted for unknown id"
    );
    assert!(
        TestHarness::channel_count(&snapshot, "registry.theme.fallback_used") > 0,
...
fn phase3_identity_registry_sign_success_emits_identity_channels() {
    ...
    let snapshot = harness.snapshot();
    assert!(
        TestHarness::channel_count(&snapshot, "registry.identity.sign_started") > 0,
        "identity sign started channel should be emitted"
    );
    assert!(
        TestHarness::channel_count(&snapshot, "registry.identity.sign_succeeded") > 0,
        "identity sign succeeded channel should be emitted"
    );
    assert_eq!(
        TestHarness::channel_count(&snapshot, "registry.identity.sign_failed"),
        0,
...
fn phase3_identity_registry_key_unavailable_emits_failure_channels() {
    ...
    assert!(
        TestHarness::channel_count(&snapshot, "registry.identity.sign_failed") > 0,
        "identity sign failed channel should be emitted"
    );
    assert!(
        TestHarness::channel_count(&snapshot, "registry.identity.key_unavailable") > 0,
...
fn diagnostics_channel_config_update_emits_config_changed_channel() {
    ...
    let snapshot = harness.snapshot();
    assert!(
        TestHarness::channel_count(&snapshot, registries::CHANNEL_DIAGNOSTICS_CONFIG_CHANGED) > 0,
