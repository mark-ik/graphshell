...
fn startup_env_snapshot_emits_channel_when_env_set() {
    ...
    assert!(
        TestHarness::channel_count(&snapshot, registries::CHANNEL_STARTUP_CONFIG_SNAPSHOT) > 0,
...
fn history_manager_limit_env_emits_channel() {
    ...
    let snapshot = harness.snapshot();
    assert!(
        TestHarness::channel_count(&snapshot, registries::CHANNEL_UI_HISTORY_MANAGER_LIMIT) > 0,
...
fn verse_preinit_calls_emit_channel() {
    ...
    let snapshot = harness.snapshot();
    assert!(
        TestHarness::channel_count(&snapshot, registries::CHANNEL_VERSE_PREINIT_CALL) > 0,
