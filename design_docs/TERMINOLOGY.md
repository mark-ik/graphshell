...
* **FrameTabSemantics**: A planned optional semantic overlay on top of the `egui_tiles` structural tree. Its intended role is to persist semantic tab group membership so that meaning is not lost when `egui_tiles` simplification restructures the tree. When implemented, it is expected to serialize into the frame bundle as frame state, not WAL data.
