...
* **Tile** (graph-citizenship): A Pane whose address has been written to the graph as a `Node` — i.e., a promoted pane. The tab bar materializes as the visual symptom of that address being in the graph. `TileKind::Node(NodePaneState)` is the promoted content tile. `TileKind::Graph` and `TileKind::Tool` are also tiles in this sense: their `graphshell://` addresses resolve to graph nodes and they carry tab bars. A collection of Tiles grouped into a Frame corresponds to a bounded region of nodes in the graph canvas, visually represented by a titled, colored MagneticZone on the canvas.
* **Graphshell address scheme**: Internal tile types use a `graphshell://` address namespace so that their graph presence follows the same address-as-identity rule as web content tiles. Canonical forms:
    * `graphshell://view/<GraphViewId>` — a Graph View pane node.
    * `graphshell://tool/<name>` — a tool/subsystem pane node. If multiple instances of the same tool are open simultaneously, a numeric discriminator is appended: `graphshell://tool/<name>/<n>` (starting at 2; the discriminator is recycled upon pane closure).
    * `graphshell://frame/<FrameId>` — a Frame node. Frame nodes carry edges to all member tile nodes; on the canvas, a Frame node drives a titled, colored MagneticZone attracting its member nodes.
    * `graphshell://settings/<section>` — a Settings pane node (planned).
