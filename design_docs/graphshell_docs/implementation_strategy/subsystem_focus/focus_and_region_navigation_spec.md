...
`Escape` is therefore reserved for dismiss/back semantics. When no transient surface,
capture owner, or explicit scoped back path is active, bare `Escape` should do nothing
rather than toggling a persistent shell/view mode or opening a control surface.
