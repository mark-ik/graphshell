...
**Date**: 2026-02-22
**Status**: In progress (core named-workspace manifest migration implemented; follow-on cleanup remains)
...
- **Workbench**: the tiling system and tile-tree UI framework (the layout engine / surface)
- **Workspace**: one persisted or live arrangement shown in the workbench
...
- [ ] Add a temporary startup warning/log for legacy NodeKey-based named workspaces (optional but recommended)
...
- [ ] Revisit which reserved workspaces participate in membership cache (likely still excluded)
- [ ] Ensure retention actions continue to exclude reserved workspaces by policy
...
### Stage 8 (Planned): Tab Semantics Metadata + Simplify-Safe Restore

Depends on:

- Stages 1-4 (manifest model in place)

Goal:

- preserve Graphshell tab behavior while allowing `egui_tiles::simplify()` to operate normally on
  the workbench tree.

Primary file targets:

- `desktop/persistence_ops.rs`
- tab/grouping helpers under `desktop/*`
- tab-aware UI/query consumers (`toolbar_omnibar`, pin UI, etc.)
- workspace plan/docs comments (as needed)

Tasks:

- [ ] Extend workspace bundle schema with tab semantics metadata (group membership/order/active tab)
- [ ] Emit tab semantics metadata from save path
- [ ] Reapply tab semantics during restore (including single-pane rewrap when requested)
- [ ] Update tab-aware consumers to prefer tab semantics metadata over pure shape inference where appropriate
- [ ] Add/validate the single-pane "inverted tab button" affordance for on-demand rewrap
- [ ] Introduce explicit promotion/demotion APIs (semantic tab/group <-> pane rest state)
- [ ] Add exact user-facing warning/repair messaging for invalid tab metadata repairs

Acceptance gates:

- single-tab workspaces remain tab-discoverable after save/restore even if structural simplification occurs
- tab order and active-tab state survive roundtrip
- workbench restore can simplify structurally without losing Graphshell tab semantics
- dependency-friendly simplification no longer conflicts with app-level behavior
- demoted semantic tabs/groups can be promoted back to tabs with metadata restored intact
...
- Stage 8 expands scope into explicit tab semantics and simplify-safe tab UX; it is a workbench/tab
  behavior improvement after the core manifest migration.
...
14. **(If Stage 8 implemented) Simplify-safe tab semantics** preserve tab discoverability and tab metadata roundtrip independent of tree-shape collapse.
15. **(If Stage 8 implemented) Promotion semantics** support pane rest states and on-demand rewrap/promotion with exact repair warnings when metadata is corrected.
...
- Stage 4 (primary-path cleanup; legacy helper retained only for transition tests)
- Stage 6 (UUID-keyed recency + startup recency seeding from bundle metadata)
- Stage 7 partial (debug tracing for resolver + restore paths)
- Stage 5 partial:
...
- Stage 5 (remaining cleanup/policy review only; core reserved/session/pin bundle behavior is in place)
- Stage 7 test/assertion surface for reason-order traces
- optional Stage 4 cleanup of legacy-format transition test helpers
