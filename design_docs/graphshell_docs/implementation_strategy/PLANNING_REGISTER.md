...
| Lane | Scope | Status | Primary Docs / Hotspots | Notes |
| --- | --- | --- | --- | --- |
| `lane:knowledge-capture` (`#98`) | Graph enrichment: tags, badges, UDC classification, import/clip enrichment, visible graph effects | Prospective | `canvas/2026-03-11_graph_enrichment_plan.md`, `canvas/2026-02-23_udc_semantic_tagging_plan.md`, `canvas/2026-02-24_layout_behaviors_plan.md`, `canvas/2026-02-20_node_badge_and_tagging_plan.md`, `2026-02-11_*_plan.md` | Canonical “capture + classify + surface” lane. Prototype-first rule: no agent/sync breadth before inspector/filter legibility and durable provenance carrier closure. Prefer relation-family + Navigator extensions over bespoke imported/file-tree side models. |
| `lane:layout-semantics` (`#99` — to be filed) | Layout injection hook, frame-affinity organizational behavior (legacy alias: Magnetic Zones); workbench/workspace/tile semantic distinctions | Prospective — **first execution slice now authorized (2026-03-18)**: `FrameAffinityRegion` data model + post-physics force hook + backdrop rendering per `canvas/layout_behaviors_and_physics_spec.md §4` | `canvas/layout_behaviors_and_physics_spec.md`, `canvas/2026-03-14_graph_relation_families.md`, `2026-02-24_layout_behaviors_plan.md`, `2026-02-22_workbench_tab_semantics_overlay_and_promotion_plan.md` | `ArrangementRelation(FrameMember)` edges are live in the relation-family model. First slice: derive `FrameAffinityRegion` from those edges, apply soft-force through post-physics hook, render backdrop below nodes. Tie `FamilyPhysicsPolicy.arrangement_weight` to the force magnitude. Lane ID `#99` to be filed before execution. |
