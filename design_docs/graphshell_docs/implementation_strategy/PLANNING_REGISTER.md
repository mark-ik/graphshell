...
- [aspect_render/2026-04-12_rendering_pipeline_status_quo_plan.md](aspect_render/2026-04-12_rendering_pipeline_status_quo_plan.md) - Code-verified rendering-pipeline execution anchor: current compositor status, current graph-renderer status, and the real order of compositor stabilization, graph optimization, `graph-canvas` extraction, and backend follow-on work.
- [aspect_projection/ASPECT_PROJECTION.md](aspect_projection/ASPECT_PROJECTION.md) - Policy authority for the Projection aspect: how domains represent themselves in each other without transferring truth ownership; graphlet is the first canonical projection object but not the only one.
- [aspect_projection/projection_interdomain_contract_spec.md](aspect_projection/projection_interdomain_contract_spec.md) - General projection contract for linked/unlinked hosting, descriptor/invalidation rules, and cross-domain correspondence beyond graphlet-only workflows.
- [aspect_projection/projection_runtime_lifecycle_spec.md](aspect_projection/projection_runtime_lifecycle_spec.md) - Runtime contract for projection descriptor keys, invalidation families, refresh behavior, and lifecycle events so graphlet can act as the first clean reference implementation.
- [aspect_distillery/ASPECT_DISTILLERY.md](aspect_distillery/ASPECT_DISTILLERY.md) - Policy authority for the Distillery aspect: policy-gated transformation from approved local graph/agent sources into typed intelligence artifacts under security and agent-supervision constraints.
- [aspect_distillery/distillation_request_and_artifact_contract_spec.md](aspect_distillery/distillation_request_and_artifact_contract_spec.md) - Companion contract for distillation request objects, transform-family execution, typed artifact classes, and promotion boundaries into STM/LTM and transfer packaging.
- [aspect_distillery/semantic_scene_scaffolding_note.md](aspect_distillery/semantic_scene_scaffolding_note.md) - Distillery design note for non-gimmicky automatic scene generation: semantic scene scaffolding, capability tiers, candidate artifact classes, and the main UX/architecture blockers.
...
| 5 | ✅ **Frame-affinity organizational behavior / Group-in-a-Box / Query-to-Zone** (legacy alias: Magnetic Zones) | Landed as graph-backed frame-affinity behavior with post-physics force application, backdrop rendering, and `zones_enabled` gating. | `2026-02-24_layout_behaviors_plan.md` Phase 3, `2026-02-18_graph_ux_research_report.md` §13.1 | Closed under `lane:layout-semantics` after the layout extension path and graph-first frame semantics were resolved; treat the plan doc as historical execution record rather than an open blocker list. |
...
| 3 | ✅ **Reheat physics on `AddNode` / `AddEdge`** | Done — structural graph mutations reheat runtime physics instead of leaving the graph visually inert while paused. | `2026-02-24_layout_behaviors_plan.md` §1.1, `2026-02-18_graph_ux_research_report.md` §5.3 |
| 4 | ✅ **Spawn new nodes near semantic parent (parent + jitter)** | Done — anchored placement now backs child-webview creation and generic anchored new-node placement, with center/centroid fallback only when no anchor is available. | `2026-02-24_layout_behaviors_plan.md` §1.2, `2026-02-18_graph_ux_research_report.md` §§2.1, 2.6 |
