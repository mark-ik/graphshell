...
| --- | --- | --- | --- | --- |
| Graph canvas camera controls fail globally | `pan drag`, `wheel zoom`, `zoom in/out/reset`, and `zoom-to-fit` fail in the default graph pane (not just multi-pane) | `render/mod.rs`, `app.rs`, `shell/desktop/ui/gui.rs`, `input/mod.rs` | Recent camera-targeting fixes landed; latest stabilization slice (`b77fd1e`) removes `radial_open` pan gating to prevent sticky radial-menu state from blocking background pan while preserving lasso/right-click guards. | Default graph pane supports pan + wheel zoom + zoom commands again; repro is closed with targeted tests/diagnostics. |
