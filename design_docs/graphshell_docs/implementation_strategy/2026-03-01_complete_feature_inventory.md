...
| FrameTabSemantics persistence | In progress | Persisted frame bundles now carry optional semantic tab metadata with load-time repair, explicit repair/resave command plumbing, saved pane-rest bundles can rewrap into visual tabs on restore, and a runtime semantic overlay now powers live restore/collapse intents plus collapsed-group save preservation; workbench-host `Tabs` / `Rest` affordances and focused-pane keyboard toggle are landed, while stricter lifecycle confirmation and broader consumer migration are still pending |
