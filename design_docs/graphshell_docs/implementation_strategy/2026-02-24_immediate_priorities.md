...
| 3 | **Registry Phase 6.4 canonical import/path closure** | Path shims and mixed imports increase churn and hide boundary regressions during feature work. | `2026-02-22_registry_layer_plan.md`, this doc §5.3 | Finish `persistence`/service path canonicalization subtree slices with compile + contract tests after each slice. |
| 4 | **Registry Phase 6.5 shim removal + final boundary lock + doc path sync** | Final architecture cleanliness gate before scaling feature work; removes transitional ambiguity. | `2026-02-22_registry_layer_plan.md`, `CODEBASE_MAP.md`, `ARCHITECTURAL_OVERVIEW.md`, this doc §5.4 | Delete root shims, tighten mutator visibility, full suite pass, docs reflect canonical paths only. |
| 5 | **Pane-hosted multi-view architecture (generalize "multi-graph pane")** | Multiple current plans assume panes host graph views, Servo/Wry viewers, and tool surfaces; the plan needs one canonical pane-view model first. | `2026-02-22_multi_graph_pane_plan.md`, `GRAPHSHELL_AS_BROWSER.md`, `2026-02-23_wry_integration_strategy.md`, `2026-02-24_universal_content_model_plan.md` | Define pane view capsule/descriptor model and lifecycle rules; preserve `GraphViewState` as graph-pane payload, not universal pane state. |
