...
| `registries/infrastructure/mod_activation.rs` + `mod_loader.rs` | **REGISTRY** (mod registration is the canonical extension seam) — but blocked by `crate::mods::wasm::activate_mod_headless` / `deactivate_mod_headless` direct calls inside the activation closure (Slice 68 finding). Cleanly extracting requires designing a `WasmModRuntime` trait so the host injects the runtime as DI; that's design work, not mechanical extraction. **Deferred.** mod_activation.rs is intrinsically host-side (hard-codes function pointers to `crate::mods::native::*::activate`) and stays in tree regardless. | `registrar/register-mod-loader/` *(deferred — needs WasmModRuntime trait design)* |
