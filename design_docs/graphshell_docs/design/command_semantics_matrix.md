...
| `NodeDelete` | Delete Selected | Node | per-View | At least one selected or target node | `GraphIntent::RemoveSelectedNodes` | Soft | Keyboard, Palette, Radial | Greyed + reason when no node context | `Delete` |
...
| `NodeOpenSplit` | Open Split | Tile | per-Frame | At least one selected or target node | Requests node open in split-horizontal mode | Yes | Palette, Radial | Greyed + reason when no node context | None |
...
| `NodeMoveToActivePane` | Move to Active Pane | Node/Tile | per-Frame | At least one selected or target node | Routes node into active pane context | Yes | Palette, Radial | Greyed + reason when no node context | None |
