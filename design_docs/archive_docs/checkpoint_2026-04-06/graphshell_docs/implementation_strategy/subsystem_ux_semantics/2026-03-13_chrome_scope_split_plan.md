...
- Landed: the graph-scoped host now renders a compact `GraphViewSlot` strip
  with focus/open/reorder affordances and surfaces active graph-view filter
...
2. Implement "Open with…" picker on pane row expand using the projected
  candidate viewer IDs carried with each pane-row/node-viewer summary.
3. Route backend swaps through `WorkbenchIntent::SwapViewerBackend` in the
  apply layer.
...
- Landed: settings-page routing reads `WorkbenchLayerState` to decide transient
  overlay (`GraphOnly` / `GraphOverlayActive`) vs tiled workbench pane; return-target
  capture, diagnostics emission, and both paths are test-covered.
- Slice 6 is fully landed.
